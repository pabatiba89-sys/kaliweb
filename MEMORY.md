...
- 2026-09-23：热点跟踪严格分两套数据：左侧“热点聚合”使用糖果指数 `GET /api/hotlist/list`，并展示其 `hotScore`；右侧“媒体热点”使用 `POST /api/hotlist/search` 的 `REALTIME` 模式，按新闻/媒体来源动态分组。右侧直接删除索比光伏、储能中国网、能源界、国家能源局和电视猫来源，并将抖音、法广、YouTube、CNN 按该顺序固定置顶；不得用搜索接口替换糖果指数。
- 2026-09-21：Asset Studio 图生数字人统一使用 `imageUrl` 提交已生成图片或本地上传图片，不再提交 `generatedImageId`；图片列表的通用标识可能是任务 ID，不能假定为生成图片记录的整数 ID。此规则取代 2026-07-09 关于必须传生成图片 ID 的旧约定。
- 2026-09-16：“创建智能包装”、“形象播报 Pro”和“素材成片 Pro”与普通数字人/混剪模式统一支持 `useBackgroundMusic/useCover` 开关；关闭后必须隐藏对应配置、跳过封面校验与上传、清空旧媒体字段，并将关闭规则同时写入顶层和闪剪参数。只读后台的五条相关创建路径需按 `docs/video-creator-optional-media-contract.md` 统一适配，否则最终成片仍可能强制配乐或封面。
- 2026-09-15：数字人口播和真人口播的“选择已配置”入口必须稳定显示，不能由预设列表是否已加载或是否为空决定；`/api/team-video-preset/list` 在团队手机号缺失时也要发起请求，手机号仅作为可选筛选参数，无配置时由选择弹窗展示空状态。
- 2026-09-15：代理分佣的用户可见界面暂时关闭：账户菜单不展示代理中心，登录框不展示邀请码手填入口，也不展示邀请绑定提示；推广链接的静默归因、代理页代码和后台数据均保留。统一通过 `VITE_AFFILIATE_UI_ENABLED=true` 恢复可见界面。
- 2026-09-14：Asset Studio 的个人形象视频和声音训练素材时长统一为 30–120 秒（含边界）；文件选择、录音结束和提交前都需校验，无法读取时长时拒绝提交，授权视频规则不随此需求改变。只读后台训练接口当前仅接收媒体 URL、没有时长字段或 30–120 秒校验，因此网页端可落实交互限制，但服务端强校验仍需后台补充媒体时长探测或可信元数据校验。
...
- 2026-09-11：视频包装模板选择弹窗必须按 9:16 与 16:9 分栏筛选，且每次打开默认 9:16；比例筛选属于共用资源弹窗能力，需同时覆盖数字人口播、混剪、真人口播和团队包装预设入口，翻页追加后也不能混入另一比例。
- 2026-09-11：音乐视频制作弹窗需同时填写歌曲名称（最多 80 字符）和作者（最多 50 字符），前端提交 `title + author`；歌词输入区应显式提供“清除提示内容”，最终生成请求仍自动使用清理后歌词。
- 2026-09-11：音乐详情和音乐视频查询都返回“任务详情对象 + `items`”；响应解析必须优先保留含 `task_id/status/related/video_url`等标识的详情对象，不能因为存在空 `items: []` 就把整个响应取成空对象，否则会丢失视频地址和多首成品。
- 2026-09-10：音乐详情页的每个成品歌曲版本都应常驻显示对应的音乐视频位置；点击制作时必须填写最多 50 字符的作者姓名并作为 `author` 提交。歌词模式不再提供 `[Verse]`/`[Chorus]` 等提示标记，预填、重试和最终生成请求均需清理中英文段落提示词。
- 2026-09-07：团队包装预设中的声音标识必须选择并保存声音资产的 `speakerId`，不能使用训练记录的 `voiceId`；旧预设回显需同时兼容两种标识，并在匹配资产后转换为 `speakerId` 再用于视频制作。
...
- 2026-07-12：工作台页面不要展示提示性说明条；首页趋势面板不展示糖果指数、热度分或类似分值，只保留排名、来源、分类和话题。热点跟踪页的聚合榜依后续规则展示糖果指数 `hotScore`。新增首页工作流文案后需同步本地化覆盖并清理旧首页词条。
...
- 2026-09-14：AI Video Lab 的成片详情和任务详情改为展示完整提示词并提供复制入口；生成类动作按“重试 / 编辑后重做 / 制作续集”区分，均先进入创建页确认、重新试算积分，不能在详情页直接提交或扣费。“重试”复用原提示词与全部可恢复配置，“编辑后重做”聚焦提示词修改，“制作续集”仅对成功成片显示，首版以当前成片作为参考视频进入可编辑的参考生视频路径。现有 `/api/team-notion/publish-video` 只接受 `video_production_tasks.id`，不得把 `generated_ai_videos.id` 直接传入，否则存在同号误发布风险；AI 成片发布使用专用 `/api/team-notion/publish-ai-video`。
...
- 2026-09-14：发布中心的 Kali Publish 开源说明固定显示在主要操作区上方；系统发布账号为空时引导进入“发布设置”，不再把开源说明塞进账号选择区。本机服务未启动或本机账号为空时，发布设置的本机账号管理区也必须展示同一开源下载入口。
- 2026-09-26：Music Studio “风格补充”输入框不在前端设定字符数上限；实际生成仍受上游音乐模型对完整 `style` 字段的限制。
