...
- 2026-07-12：支付方案初步倾向使用香港主体接入 Stripe；中国大陆销售地区用户引导至喀理小程序支付，香港及其他国家/地区在 Web 端使用 Stripe。支付路由不能只按界面语言判断，应以用户选择的销售地区/账单地区为主，语言和 IP 只用于默认推测，并允许用户修改。
...
- 2026-07-12：工作台 Billing 按 Cloudflare IP 地区分流：Worker 的 `/site-region` 返回 `request.cf.country`，中国大陆 `CN` 显示喀理小程序码及微信购买/续费/账单引导，其他地区保留海外账单与套餐页。小程序码静态资产为 `public/payments/kali-mini-program.png`。
