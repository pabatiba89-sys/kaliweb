...
- 2026-07-24：视频制作页重新选择数字人后，提交给后端的数字人业务 ID 必须优先使用资源返回的 `aihuman_id`（兼容 `aihumanId/ai_human_id`），不能用列表通用 `id` 或训练任务 ID 代替；提交体需同时带 `aiHumanId/aihuman_id/ai_human_id/virtualmanId` 别名。
