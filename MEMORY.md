...
- 2026-09-13：发布账号改为显式匹配：工作台“发布设置”读取本机 5409 的 `/getValidAccounts`，在当前浏览器保存“Kali 团队发布账号 ID -> 本机平台类型 + 账号 ID”的关系，不保存 cookie。发布中心、视频详情和 AI Video Lab 在创建 Kali 发布任务前都必须按 ID 校验匹配；未匹配或本机账号重新登录导致 ID 失效时直接阻断，禁止回退到同名匹配。平台编号固定为 1 小红书、2 视频号、3 抖音、4 快手、5 TikTok、6 YouTube。
