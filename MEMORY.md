...
- 2026-07-17：Pro 分镜字幕输入框内输入回车或粘贴换行文本时，应自动把换行内容拆成多个分镜；第一段留在当前分镜，后续段落追加为新分镜，末尾回车追加空分镜。
...
- 2026-07-18：解析 `payload_json` 时不能因为顶层有全局 `materials` 就停止下钻，必须先查找嵌套 `productionPayload/draftPayload/requestPayload/createPayload/jsonPayload.sceneList`；若分镜只存素材 ID/URL 引用，则用全局素材按 ID/URL 匹配回该分镜。
