...
- 2026-07-16：Video Studio 新增“专业口播混剪”作为独立生产类型，对应闪剪 OpenAPI `POST /v1/clip/video/custom_virtualman_broadcast`；前端需构造 `styleId/virtualmanId/speakerId/speakerExtra/scenes/introduceCard/packRules/processRules/structLayers`，并通过现有 `/api/video-mix/create` 后端代理提交，不与普通混剪 `oralMixCutting` 共用字段假设。
