...
- 2026-09-22：热点前端统一使用 `POST /api/hotlist/search` 的 `REALTIME` 模式，根分类只传“新闻/媒体”；保持原热点页面布局，媒体来源按实时返回数据动态分组，新页面不再调用兼容接口 `/api/hotlist/list`。产品层直接删除索比光伏、储能中国网、能源界、国家能源局和电视猫来源，并将抖音、法广、YouTube、CNN 按该顺序固定置顶。
