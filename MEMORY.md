...
- 2026-07-28：价格统一以人民币套餐价为锚点；`zh-CN`/`zh-TW` 保留原人民币金额并显示 `¥`，其他界面语言按 `USD = RMB / 7.2` 四舍五入为整数并显示 `$`。Evonet 接口和请求体不变，前端固定提交 `currency=USD`，后端创建订单时执行相同换算并只允许 USD，保证展示金额与实际扣款一致。
