...
function VideoCreatorPage({ authVersion, usePrefill, productionType = 'oral', backLabel = '返回视频管理', onBack, onLogin, onCreated, onChangeProductionType }) {
  ...
  const changeSceneContent = (sceneId, value, inputType = '') => {
    const normalized = value.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    setScenes((current) => {
      const sceneIndex = current.findIndex((scene) => scene.id === sceneId);
      if (sceneIndex < 0) return current;
      const sourceScene = current[sceneIndex];
      const previousContent = String(sourceScene.content || '').replace(/\r\n/g, '\n').replace(/\r/g, '\n');
      const previousNewlineCount = (previousContent.match(/\n/g) || []).length;
      const nextNewlineCount = (normalized.match(/\n/g) || []).length;
      const insertedNewline = nextNewlineCount > previousNewlineCount && !String(inputType).startsWith('delete');
      if (!insertedNewline) return current.map((scene, index) => (index === sceneIndex ? { ...scene, content: value } : scene));
      const hasTrailingNewline = /\n\s*$/.test(normalized);
      const chunks = normalized
        .split(/\n+/)
        .map(cleanGeneratedScene)
        .filter(Boolean);
      if (hasTrailingNewline) chunks.push('');
      const nextChunks = chunks.length ? chunks : [''];
      const insertedScenes = nextChunks.slice(1).map((content) => createCreatorScene(content));
      return current.flatMap((scene, index) => {
        if (index !== sceneIndex) return [scene];
        return [{ ...sourceScene, content: nextChunks[0] || '' }, ...insertedScenes];
      });
    });
  };
