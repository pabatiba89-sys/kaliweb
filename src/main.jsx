...
import { buildCreatorOptionalMedia, resolveCreatorOptionalMedia } from './videoCreatorOptions';
...
const legalDocuments = {
  ...
  'legal-privacy': {
    ...
    sections: [
      ...
      ['6. 执法机关与政府请求', '我们会逐案审查执法机关或政府部门提出的数据请求。只有在请求具有有效法律程序、范围具体明确且符合必要性与比例原则时，才会依法披露相应数据；无效、非正式或过度宽泛的请求可能被拒绝、质疑或要求缩小范围。'],
    ],
  },
  'legal-voice': {
...
const REALMAN_VIDEO_STUDIO_TYPE = {
  ...
  icon: Cuboid,
  description: '查看智能包装任务、成片状态和发布流程。',
  sectionHint: '本地视频、AI 成片、包装模板与补充素材',
  title: '真人视频包装',
...
function AIVideoLabPage({ authVersion, language, onLogin, onOpenBilling, onOpenPromptAssistant, onOpenPackaging, promptDraft }) {
  ...
  const openDetailDraft = (draft, { focusPrompt = false, message = '' } = {}) => {
    ...
    setReferences({
      ...draft.references,
      videos: missingVideoDuration ? draft.references.videos.filter((item) => Number(item.duration) > 0) : draft.references.videos,
    });
    setUrlDrafts({ images: '', videos: '', audios: '', videoDuration: '' });
    setQuote(null);
    setQuoteError('');
    setDetail(null);
    setTab('create');
  ...
  const openAIVideoPublish = async () => {
    if (!detail?.videoId || detail.kind !== 'video' || detail.status.key !== 'success' || !detail.videoUrl) return;
    setPublishTarget(detail);
    setPublishTitle('');
    setPublishTopics('');
  ...
  const publishAIVideo = async () => {
    ...
    if (!account) {
      setPublishState((current) => ({ ...current, message: '请选择发布账号' }));
      return;
    }
    const isNow = publishMode === 'now';
    if (!isNow) {
      const scheduledAt = new Date(publishAt);
      if (!publishAt || Number.isNaN(scheduledAt.getTime())) {
        setPublishState((current) => ({ ...current, message: '请选择发布时间' }));
        return;
      }
      if (scheduledAt.getTime() <= Date.now()) {
        setPublishState((current) => ({ ...current, message: '定时发布时间必须晚于当前时间' }));
        return;
      }
    }
  ...
  return (
    <div className="ai-video-page">
      ...
      {publishDialog && <VideoActionDialog className="ai-video-publish-layer" title="发布 AI 视频" description="填写对外标题和话题，选择发布账号与时间。" busy={publishState.busy} submitLabel={publishState.localUnavailable ? '检测并继续发布' : '确认发布'} onClose={closeAIVideoPublish} onSubmit={publishAIVideo}>
        ...
        <label className="video-dialog-field"><span>发布账号</span><select value={publishAccountId} onChange={(event) => { setPublishAccountId(event.target.value); setPublishState((current) => ({ ...current, message: '' })); }} disabled={publishState.loading}><option value="">{publishState.loading ? '正在加载发布账号…' : '请选择发布账号'}</option>{publishAccounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
...
const normalizeAIVideoPublishAccount = (item, index) => {
  const source = videoObject(item);
  const account = normalizePublishAccount(source, index);
  return {
    ...account,
    id: videoText(source.publishAccountId, source.publish_account_id, source.accountId, source.account_id, source.id),
  };
};
...
function VideoActionDialog({ className = '', title, description, children, busy, submitLabel, onClose, onSubmit }) {
  return (
    <div className={`video-action-layer${className ? ` ${className}` : ''}`} role="dialog" aria-modal="true" aria-label={title}>
...
function AIVideoPackagingPage({ sourceVideo, authVersion, language, onBack, onLogin, onCreated }) {
  ...
  const sourcePreviewUrl = activeSourceVideo?.previewUrl || sourceUrl;
  const token = getAccessToken();
  const [form, setForm] = useState({
    title: (sourceVideo?.videoUrl || sourceVideo?.video_url || sourceVideo?.url) ? getAIVideoDialogueTitle(sourceVideo?.prompt, 'AI 真人视频') : '',
    topic: normalizeAIVideoTopics(sourceVideo?.topic || sourceVideo?.topics || sourceVideo?.tags || sourceVideo?.raw?.tags).join('、'),
    language: VOICE_LANGUAGE_OPTIONS.some((option) => option.value === language) ? language : 'zh-CN',
  });
  const [selected, setSelected] = useState({ videoTemplate: {}, coverTemplate: {}, music: {} });
  const [optionalMedia, setOptionalMedia] = useState({ useBackgroundMusic: true, useCover: true });
  const [cover, setCover] = useState(null);
  const [materials, setMaterials] = useState([]);
  const [resources, setResources] = useState({ videoTemplate: [], coverTemplate: [], music: [], material: [], aiVideoMaterial: [], aiImageMaterial: [] });
  ...
  useEffect(() => { sourceVideoRef.current = activeSourceVideo; }, [activeSourceVideo]);
  useEffect(() => { coverRef.current = cover; }, [cover]);
  useEffect(() => () => {
    const source = sourceVideoRef.current;
    if (source?.origin === 'local' && source.previewUrl) URL.revokeObjectURL(source.previewUrl);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [templateResult, coverTemplateResult, materialResult, aiVideoResult, aiImageResult, musicResult] = await Promise.all([
        ...
        apiFetch('/api/shanjian/cover-templates', { auth: false, params: { page: 1, page_size: 100, pageSize: 100, scene: 'realMan' }, timeoutMs: 12000 }),
      ...
      const coverTemplates = getCreatorPayloadList(coverTemplateResult).map((item, index) => normalizeTemplate(item, index, '封面包装'));
      ...
      setMessage([!templateResult.ok && '真人口播模板加载失败', !coverTemplateResult.ok && '封面模板加载失败', !materialResult.ok && '素材库加载失败', !aiImageResult.ok && 'AI 图片加载失败', !musicResult.ok && '背景音乐加载失败'].filter(Boolean).join('；'));
  ...
  const chooseResource = (type, option) => {
    ...
    if (type === 'coverTemplate') {
      setSelected((current) => ({ ...current, coverTemplate: option }));
      setDialogType('');
      return;
    }
    if (type !== 'material') {
  ...
  const chooseSourceVideo = async (event) => {
    ...
    if (!file) return;
    const extension = getExtension(file.name);
    if (!VIDEO_EXTENSIONS.includes(extension) || (file.type && !file.type.startsWith('video/'))) {
      setMessage('视频仅支持 mp4、mov 格式');
      return;
    }
    try {
      const duration = await getVideoDuration(file);
      if (activeSourceVideo?.origin === 'local' && activeSourceVideo.previewUrl) URL.revokeObjectURL(activeSourceVideo.previewUrl);
      const nextVideo = {
        title: file.name,
        file,
      ...
      setActiveSourceVideo(nextVideo);
      setForm((current) => ({
        ...current,
        title: current.title.trim() ? current.title : file.name.replace(/\.[^.]+$/, '').slice(0, 80),
      }));
      setMessage('');
    } catch {
      setMessage('无法读取视频，请重新选择');
    }
  };
  ...
  const removeCoverTemplate = () => setSelected((current) => ({ ...current, coverTemplate: {} }));

  const toggleOptionalMedia = (key) => {
    setOptionalMedia((current) => ({ ...current, [key]: !current[key] }));
    if ((key === 'useBackgroundMusic' && dialogType === 'music') || (key === 'useCover' && ['cover', 'coverTemplate'].includes(dialogType))) setDialogType('');
    setMessage('');
  };
  ...
  const submit = async () => {
    ...
    if (!form.topic.trim()) { setMessage('请输入视频话题'); return; }
    if (!sourceUrl && !activeSourceVideo?.file) { setMessage('请上传需要包装的视频'); return; }
    if (!selected.videoTemplate.id) { setMessage('请选择真人口播模板'); return; }
    if (optionalMedia.useCover && !cover) { setMessage('请选择或上传视频封面'); return; }
    setBusy(true);
    setMessage('');
    setUploadProgress('');
    try {
      let submitSourceUrl = sourceUrl;
      if (activeSourceVideo?.file && !submitSourceUrl) {
        setUploadProgress('正在上传源视频…');
        const uploadResult = await uploadFile(activeSourceVideo.file, {
          source: 'realman-source-video',
          onProgress: (progress) => setUploadProgress(`正在上传源视频… ${progress}%`),
        });
        if (!uploadResult.ok) throw new Error(getResultMessage(uploadResult, '源视频上传失败'));
        submitSourceUrl = getUploadedUrl(uploadResult);
        if (!submitSourceUrl) throw new Error('源视频上传未返回地址');
        setActiveSourceVideo((current) => current?.file === activeSourceVideo.file ? { ...current, videoUrl: submitSourceUrl, uploaded: true } : current);
      }
      let coverUrl = optionalMedia.useCover ? cover?.url || '' : '';
      if (optionalMedia.useCover && cover?.file && !coverUrl) {
      ...
      const payload = buildRealmanPackagingPayload({
        sourceVideo: { ...activeSourceVideo, videoUrl: submitSourceUrl },
        title: form.title,
        topic: form.topic,
        language: form.language,
        template: selected.videoTemplate,
        music: selected.music,
        cover: cover ? { ...cover, url: coverUrl } : {},
        coverTemplate: selected.coverTemplate,
        materials,
        useBackgroundMusic: optionalMedia.useBackgroundMusic,
        useCover: optionalMedia.useCover,
      });
  ...
  const packagingSelectorRows = [
    ['videoTemplate', '真人口播模板', '请选择真人口播模板', Cuboid],
    ...(optionalMedia.useCover ? [['coverTemplate', '封面模板', '请选择封面模板', Image]] : []),
    ...(optionalMedia.useBackgroundMusic ? [['music', '背景音乐', '自动匹配背景音乐', Music2]] : []),
  ];
  return (
    <div className="video-creator-page ai-video-packaging-page">
      <header className="video-creator-header"><button className="video-back-button" onClick={onBack} disabled={busy}><ArrowLeft size={18} />{sourceVideo?.returnTo === 'video' ? '返回视频制作' : '返回成片详情'}</button><div><span>REAL PERSON VIDEO PACKAGING</span><h1>真人视频包装</h1><p>直接上传视频或使用 AI 成片，选择真人口播模板、背景音乐和补充素材后提交包装。</p></div><div className="video-creator-progress"><span className="is-done"><Check size={14} />成片</span><i /><span className="is-done"><Check size={14} />包装</span><i /><span>提交</span></div></header>
      {!token ? <div className="video-empty-state"><Cuboid size={38} /><strong>登录后开始包装</strong><p>登录后才能读取模板、素材并提交真人视频包装任务。</p><button className="primary-button" onClick={onLogin}>登录</button></div> : <div className="video-creator-layout"><main className="video-creator-main">
        <section className="video-creator-section"><div className="video-creator-section__head"><span>01</span><div><h2>选择需要包装的视频</h2><p>AI 成片会自动带入，也可以上传本地视频替换；其他包装规则保持不变。</p></div></div>{sourcePreviewUrl ? <div className="ai-video-packaging-source"><video src={sourcePreviewUrl} poster={activeSourceVideo?.lastFrameUrl || undefined} controls playsInline /><div><strong>{form.title || activeSourceVideo?.title || '待包装视频'}</strong><dl><div><dt>来源</dt><dd>{activeSourceVideo?.origin === 'local' ? '本地上传' : 'AI Video Lab'}</dd></div><div><dt>规格</dt><dd>{[activeSourceVideo?.duration ? `${formatDuration(activeSourceVideo.duration)}` : '', activeSourceVideo?.resolution?.toUpperCase(), activeSourceVideo?.aspectRatio].filter(Boolean).join(' · ') || '—'}</dd></div></dl><label className="ai-video-packaging-source-action"><Upload size={16} />更换视频<input type="file" accept="video/mp4,video/quicktime" onChange={chooseSourceVideo} /></label></div></div> : <label className="ai-video-packaging-source-empty"><FileVideo size={32} /><strong>上传需要包装的视频</strong><small>支持 mp4 / mov</small><span><Upload size={16} />选择视频</span><input type="file" accept="video/mp4,video/quicktime" onChange={chooseSourceVideo} /></label>}<div className="video-creator-fields"><label><span>视频标题 <em>必填</em></span><input maxLength={80} value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="AI 成片默认使用第一句对白，本地视频可自行修改" /><small>{form.title.length}/80</small></label><label><span>话题 <em>必填</em></span><input maxLength={80} value={form.topic} onChange={(event) => setForm((current) => ({ ...current, topic: event.target.value }))} placeholder="例如：职场沟通、办公室幽默" /><small>{form.topic.length}/80</small></label><label><span>视频语种</span><select translate="no" value={form.language} onChange={(event) => setForm((current) => ({ ...current, language: event.target.value }))}>{VOICE_LANGUAGE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label} · {option.value}</option>)}</select></label></div></section>
        <section className="video-creator-section"><div className="video-creator-section__head"><span>02</span><div><h2>选择包装配置</h2><p>真人口播模板必选；背景音乐和封面可按需关闭。</p></div>{optionalMedia.useCover && selected.coverTemplate.id && <button className="video-creator-preset-button" onClick={removeCoverTemplate}>清除封面模板</button>}{optionalMedia.useBackgroundMusic && selected.music.id && <button className="video-creator-preset-button" onClick={() => setSelected((current) => ({ ...current, music: {} }))}>改为自动音乐</button>}</div><div className="video-creator-option-toggles"><article><span><Music2 size={19} /></span><div><strong>背景音乐</strong><small>{optionalMedia.useBackgroundMusic ? (selected.music.title || '自动匹配背景音乐') : '关闭'}</small></div><button className={optionalMedia.useBackgroundMusic ? 'is-on' : ''} type="button" role="switch" aria-checked={optionalMedia.useBackgroundMusic} aria-label="是否开启背景音乐" onClick={() => toggleOptionalMedia('useBackgroundMusic')}><i /><em>{optionalMedia.useBackgroundMusic ? '开启' : '关闭'}</em></button></article><article><span><Image size={19} /></span><div><strong>封面</strong><small>{optionalMedia.useCover ? (selected.coverTemplate.title || cover?.title || '封面图片') : '关闭'}</small></div><button className={optionalMedia.useCover ? 'is-on' : ''} type="button" role="switch" aria-checked={optionalMedia.useCover} aria-label="是否使用封面" onClick={() => toggleOptionalMedia('useCover')}><i /><em>{optionalMedia.useCover ? '开启' : '关闭'}</em></button></article></div><div className="video-creator-select-list">{packagingSelectorRows.map(([key, label, hint, Icon]) => { const value = selected[key]; return <button type="button" key={key} onClick={() => setDialogType(key)}><i><Icon size={19} /></i><span><strong>{label}</strong><small>{value.title || hint}</small></span>{value.cover && <img src={value.cover} alt="" />}{key === 'music' && value.audioUrl && <audio src={value.audioUrl} controls onClick={(event) => event.stopPropagation()} />}<ChevronRight size={18} /></button>; })}</div></section>
        <section className="video-creator-section"><div className="video-creator-section__head"><span>03</span><div><h2>{optionalMedia.useCover ? '封面与补充素材' : '补充素材'}</h2><p>{optionalMedia.useCover ? '封面图片必填，可本地上传或从 AI 图片选择；补充素材可选，总时长不超过 5 分钟。' : '补充素材可选，总时长不超过 5 分钟。'}</p></div></div><div className={`video-creator-upload-grid ai-video-packaging-upload-grid${optionalMedia.useCover ? '' : ' is-single'}`}>{optionalMedia.useCover && <div className="video-creator-cover"><h3>视频封面 <em>必填</em></h3>{cover ? <div className="video-creator-cover-preview"><img src={cover.previewUrl || cover.url} alt="" /><span><strong>{cover.title}</strong><small>{cover.origin === 'local' ? '本地图片' : 'AI 图片'}</small><button type="button" onClick={removeCover}><Trash2 size={15} />删除</button></span></div> : <div className="ai-video-packaging-cover-empty"><Image size={28} /><strong>请选择视频封面</strong><small>支持 jpg / png / webp</small></div>}<div className="ai-video-packaging-cover-actions"><label><Upload size={17} />{cover?.origin === 'local' ? '本地更换' : '本地上传'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseCover} /></label><button type="button" onClick={() => setDialogType('cover')}><Image size={17} />{cover?.origin === 'ai-image' ? '从 AI 图片更换' : '从 AI 图片选择'}</button></div></div>}<div className="video-creator-material-actions ai-video-packaging-material-actions"><h3>补充素材 <em>可选</em></h3><div><button type="button" onClick={() => setDialogType('material')}><Library size={18} />选择素材</button></div><small>已选 {materials.length} 个 · 总时长 {formatDuration(materialDuration)}</small></div></div>{materials.length > 0 && <div className="video-creator-material-grid">{materials.map((material) => <article key={material.id}><span>{material.previewUrl ? <img src={material.previewUrl} alt="" /> : <Video size={24} />}</span><div><strong>{material.title}</strong><small>{material.type === 'video' ? `视频 · ${formatDuration(material.duration)}` : '图片 · 2 秒'}</small></div><button onClick={() => setMaterials((current) => current.filter((item) => item.id !== material.id))} aria-label="删除素材"><X size={16} /></button></article>)}</div>}</section>
      </main><aside className="video-creator-summary"><span>PACKAGING SUMMARY</span><h2>包装确认</h2><dl><div><dt>标题</dt><dd>{form.title || '未填写'}</dd></div><div><dt>话题</dt><dd>{form.topic || '未填写'}</dd></div><div><dt>源视频</dt><dd>{activeSourceVideo ? (activeSourceVideo.duration ? formatDuration(activeSourceVideo.duration) : '已选择') : '未选择'}</dd></div><div><dt>口播模板</dt><dd>{selected.videoTemplate.title || '未选择'}</dd></div><div><dt>封面模板</dt><dd>{optionalMedia.useCover ? (selected.coverTemplate.title || '未选择') : '关闭'}</dd></div><div><dt>视频封面</dt><dd>{optionalMedia.useCover ? (cover?.title || '未选择') : '关闭'}</dd></div><div><dt>背景音乐</dt><dd>{optionalMedia.useBackgroundMusic ? (selected.music.title || '系统自动匹配') : '关闭'}</dd></div><div><dt>补充素材</dt><dd>{materials.length} 个 / {formatDuration(materialDuration)}</dd></div></dl>{loadingResources && <div className="video-creator-uploading"><RefreshCw className="is-spinning" size={17} />正在加载包装资源…</div>}{uploadProgress && <div className="video-creator-uploading"><RefreshCw className="is-spinning" size={17} />{uploadProgress}</div>}{message && <div className={`video-list-message ${/失败|请|不能|不存在|超过|仅支持|未返回/.test(message) ? 'is-error' : ''}`}>{message}</div>}<div className="video-creator-submit ai-video-packaging-submit"><button className="primary-button" onClick={submit} disabled={busy || loadingResources}><Cuboid size={17} />{busy ? '提交中…' : '提交包装'}</button></div><p>提交后进入制作队列，可在 Video Studio 的“真人视频包装”查看进度。</p></aside></div>}
...
function VideoStudioPage({ authVersion, onLogin, onNewVideo, onCreatePackaging, initialSection = 'oral' }) {
  ...
  return (
    <div className="video-studio-page">
      <section className="video-studio-hero">
        ...
        <button className="primary-button" onClick={() => studioSection === 'realman' ? onCreatePackaging() : onNewVideo({ productionType: studioConfig.key })}><Plus size={18} />{studioConfig.createLabel}</button>
      </section>
      ...
      </> : <div className="video-empty-state"><Video size={38} /><strong>{message || (videos.length ? '没有符合筛选条件的视频' : `还没有${studioConfig.emptyLabel}制作记录`)}</strong><p>{videos.length ? '更换状态或搜索关键词后再试。' : studioSection === 'realman' ? '可直接上传视频，也可从 AI Video Lab 的成片详情进入包装。' : `点击制作，提交第一条${studioConfig.emptyLabel}。`}</p>{!videos.length && <button className="primary-button" onClick={() => studioSection === 'realman' ? onCreatePackaging() : onNewVideo({ productionType: studioConfig.key })}>{studioSection === 'realman' ? '创建智能包装' : '开始制作'}</button>}</div>}
    </div>
  );
}
...
const VIDEO_CREATOR_MATERIAL_SOURCES = [
  { key: 'library', resourceKey: 'material', label: '素材库', empty: '素材库暂无可用图片或视频' },
  { key: 'aiVideo', resourceKey: 'aiVideoMaterial', label: 'AI Video Lab', empty: '还没有 AI 视频任务' },
  { key: 'aiImage', resourceKey: 'aiImageMaterial', label: 'AI 图片', empty: '暂无生成图片' },
];

const getCreatorMaterialSource = (sourceKey = 'library') => VIDEO_CREATOR_MATERIAL_SOURCES.find((source) => source.key === sourceKey) || VIDEO_CREATOR_MATERIAL_SOURCES[0];

const normalizeCreatorAIVideoMaterial = (item = {}, index = 0) => {
  const video = normalizeAIVideoRecord(item, index);
  if (!video.videoUrl) return null;
  return {
    id: `ai-video-${video.videoId || video.id}`,
    sourceId: video.videoId || video.id,
    title: video.prompt || `AI 视频 ${index + 1}`,
    type: 'video',
    url: video.videoUrl,
    cover: video.lastFrameUrl || getVideoFrameUrl(video.videoUrl),
    previewUrl: video.lastFrameUrl || getVideoFrameUrl(video.videoUrl),
    duration: Number(video.duration) || 0,
    meta: [video.model, video.duration ? `${video.duration} 秒` : '', video.createdAt].filter(Boolean).join(' · '),
    origin: 'ai-video',
    raw: item,
  };
};

const normalizeCreatorAIImageMaterial = (item = {}, index = 0) => {
  const image = normalizeGeneratedImage(item, index);
  if (!image.url) return null;
  return {
    id: `ai-image-${image.id}`,
    sourceId: image.id,
    title: image.title || `AI 图片 ${index + 1}`,
    type: 'image',
    url: image.url,
    cover: image.url,
    previewUrl: image.url,
    duration: 2,
    meta: image.createdAt || image.prompt || 'AI 图片',
    origin: 'ai-image',
    raw: item,
  };
};
...
const getCreatorInitialState = (usePrefill) => {
  ...
  return {
    ...
    materials,
    scenes,
    optionalMedia: resolveCreatorOptionalMedia(draft, detail, payload, shanjian),
    draftId: isFailedPrefill ? '' : videoText(draft.draftRecordId, draft.draftId, draft.draft_id, detail.draftRecordId, detail.draftId, detail.draft_id, payload.draftId, payload.draft_id, draft.id, detail.id),
  };
};
...
function VideoCreatorDialog({ type, titleOverride, options, selected, loading, hasMore, loadingMore, loadMessage, materialSource = 'library', onMaterialSourceChange, onClose, onSelect, onLoadMore }) {
  ...
  return (
    <div className="video-action-layer video-creator-resource-layer" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <section className="video-creator-resource-dialog">
        ...
        <footer>{type === 'music' && <button className="outline-button" onClick={() => onSelect({ id: '', title: '', audioUrl: '' })}>不选择，自动匹配</button>}<button className="primary-button" onClick={onClose}>{type === 'material' ? `完成选择（${selected.length}）` : '关闭'}</button></footer>
      </section>
    </div>
  );
}
...
function VideoCreatorPage({ authVersion, usePrefill, productionType = 'oral', backLabel = '返回视频管理', onBack, onLogin, onCreated, onChangeProductionType }) {
  ...
  const [optionalMedia, setOptionalMedia] = useState(initial.optionalMedia);
  ...
  const [resources, setResources] = useState({ human: [], voice: [], videoTemplate: [], coverTemplate: [], music: [], material: [], aiVideoMaterial: [], aiImageMaterial: [], preset: [] });
  const [resourcePaging, setResourcePaging] = useState({
    videoTemplate: { page: 1, cursor: '', hasMore: false, loadingMore: false, message: '' },
    coverTemplate: { page: 1, cursor: '', hasMore: false, loadingMore: false, message: '' },
    material: { page: 1, cursor: '', hasMore: false, loadingMore: false, message: '' },
    aiVideoMaterial: { page: 1, cursor: '', hasMore: false, loadingMore: false, message: '' },
    aiImageMaterial: { page: 1, cursor: '', hasMore: false, loadingMore: false, message: '' },
  });
  ...
  const resourceLoadingRef = useRef({ videoTemplate: false, coverTemplate: false, material: false, aiVideoMaterial: false, aiImageMaterial: false });
  const token = getAccessToken();
  const useBackgroundMusic = optionalMedia.useBackgroundMusic;
  const useCover = optionalMedia.useCover;

  const updateForm = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const toggleOptionalMedia = (key) => {
    setOptionalMedia((current) => ({ ...current, [key]: !current[key] }));
    if ((key === 'useBackgroundMusic' && dialogType === 'music') || (key === 'useCover' && dialogType === 'coverTemplate')) setDialogType('');
    setMessage('');
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoadingResources(true);
      const [ownHumans, commonHumans, ownVoices, commonVoices, videoTemplates, coverTemplates, materialResult, aiVideoResult, aiImageResult, musicResult, userResult] = await Promise.all([
      ...
      let presets = [];
      if (needsHuman) {
        const presetResult = await apiFetch('/api/team-video-preset/list', { params: buildPackagingPresetListParams(teamPhone), timeoutMs: 10000 });
      ...
      const aiVideoItems = toList(aiVideoResult.data);
      const aiImageItems = getImageGenerationItems(aiImageResult);
      const next = {
        human: humans,
        voice: voices,
        ...
        aiVideoMaterial: aiVideoItems.map(normalizeCreatorAIVideoMaterial).filter(Boolean),
        aiImageMaterial: aiImageItems.map(normalizeCreatorAIImageMaterial).filter(Boolean),
        music,
        preset: presets,
      };
      setResources(next);
      setResourcePaging({
        ...
        aiVideoMaterial: {
          page: 1,
          cursor: '',
          hasMore: aiVideoResult.ok && aiVideoItems.length >= 24,
          loadingMore: false,
          message: aiVideoResult.ok ? '' : getResultMessage(aiVideoResult, '素材加载失败'),
        },
        aiImageMaterial: {
          page: 1,
          cursor: '',
          hasMore: aiImageResult.ok && aiImageItems.length >= 24,
          loadingMore: false,
          message: aiImageResult.ok ? '' : getResultMessage(aiImageResult, '素材加载失败'),
        },
      });
      setSelected((current) => {
  ...
  const loadMoreCreatorResources = async (type) => {
    if (!['videoTemplate', 'coverTemplate', 'material', 'aiVideoMaterial', 'aiImageMaterial'].includes(type)) return;
    const paging = resourcePaging[type];
    if (!paging?.hasMore || paging.loadingMore || resourceLoadingRef.current[type]) return;
    ...
    const isAIVideoMaterial = type === 'aiVideoMaterial';
    const isAIImageMaterial = type === 'aiImageMaterial';
    const isGeneratedMaterial = isAIVideoMaterial || isAIImageMaterial;
    const path = isMaterial ? '/api/material/list' : isAIVideoMaterial ? '/api/ai-video/videos' : isAIImageMaterial ? '/api/image-generation/images' : type === 'videoTemplate' ? '/api/shanjian/video-templates' : '/api/shanjian/cover-templates';
    const group = type === 'videoTemplate' ? (isMixed ? '混剪剪辑模板' : isCustomMixcut ? '包装混剪模板' : '视频包装') : '封面包装';
    const nextPage = paging.page + 1;
    const result = await apiFetch(path, {
      auth: isMaterial || isGeneratedMaterial,
      params: {
        page: nextPage,
        page_size: isMaterial ? MATERIAL_PAGE_SIZE : isGeneratedMaterial ? 24 : TEMPLATE_PAGE_SIZE,
        pageSize: isMaterial ? MATERIAL_PAGE_SIZE : isGeneratedMaterial ? 24 : TEMPLATE_PAGE_SIZE,
        ...(isMaterial ? { limit: MATERIAL_PAGE_SIZE, include_total: 0 } : isAIImageMaterial ? { limit: 24 } : isGeneratedMaterial ? {} : { scene: templateScene }),
        ...(paging.cursor ? (isMaterial ? { start_cursor: paging.cursor } : { sid: paging.cursor }) : {}),
      },
      timeoutMs: 12000,
    });
    const rawItems = isMaterial || isAIVideoMaterial ? toList(result.data) : isAIImageMaterial ? getImageGenerationItems(result) : getCreatorPayloadList(result);
    const normalized = rawItems
      .map((item, index) => isMaterial
        ? normalizeMaterial(item, resources[type].length + index)
        : isAIVideoMaterial
          ? normalizeCreatorAIVideoMaterial(item, resources[type].length + index)
          : isAIImageMaterial
            ? normalizeCreatorAIImageMaterial(item, resources[type].length + index)
            : normalizeTemplate(item, resources[type].length + index, group))
      .filter((item) => item && (!isMaterial || item.url));
    const existingIds = new Set(resources[type].map((item) => String(item.id)));
    const uniqueItems = normalized.filter((item) => !existingIds.has(String(item.id)));
    const nextCursor = isMaterial ? getMaterialNextCursor(result) : isGeneratedMaterial ? '' : getTemplateNextCursor(result);
      ...
      [type]: {
        ...
        hasMore: result.ok
          ? (isMaterial
            ...
            : isGeneratedMaterial
              ? uniqueItems.length > 0 && rawItems.length >= 24
              : uniqueItems.length > 0 && getTemplateHasMore({ result, cursor: paging.cursor, list: rawItems, page: nextPage }))
          : current[type].hasMore,
        loadingMore: false,
        message: result.ok ? '' : getResultMessage(result, isMaterial || isGeneratedMaterial ? '素材加载失败，请重试' : '模板加载失败，请重试'),
      },
    }));
    resourceLoadingRef.current[type] = false;
  };
  ...
  const chooseResource = (type, option) => {
    if (type === 'material') {
        ...
        setScenes((current) => current.map((scene) => {
          ...
          return {
            ...
            materials: exists
              ...
              : sceneMaterials.concat({ ...option, title: option.title, previewUrl: option.previewUrl || option.cover, duration, origin: option.origin || 'library' }),
          };
        }));
        return;
      }
      setMaterials((current) => {
        ...
        return current.concat({ ...option, title: option.title, previewUrl: option.previewUrl || option.cover, duration, origin: option.origin || 'library' });
      });
      return;
    }
  ...
  const handleSceneKeyDown = (event, sceneId) => {
    const isPlainEnter = event.key === 'Enter' && !event.shiftKey && !event.altKey && !event.metaKey && !event.ctrlKey;
    if (!isPlainEnter) return;
    if (event.nativeEvent?.isComposing || event.isComposing || event.keyCode === 229 || event.which === 229) return;
    event.preventDefault();
    if (event.repeat) return;
    splitSceneAtCursor(sceneId, event.currentTarget);
  };
  ...
  const validate = (isDraft, mode = currentProductionTypeRef.current) => {
    ...
    const validationUseCover = optionalMedia.useCover;
    ...
    if (validationUseCover && !selected.coverTemplate.id) return '请选择视频封面模板';
    if (validationUseCover && !cover) return '请上传封面图片';
  ...
  const submit = async (isDraft = false) => {
    ...
    const submitUseBackgroundMusic = optionalMedia.useBackgroundMusic;
    const submitUseCover = optionalMedia.useCover;
    ...
    try {
      let coverUrl = submitUseCover ? cover?.url || '' : '';
      if (submitUseCover && cover?.file && !coverUrl) {
      ...
      const submitScenes = [];
      const pendingMaterialCount = (isCustomMixcut ? getCreatorSceneMaterials(scenes) : materials).filter((item) => item.file && !item.url).length;
      ...
      const optionalMediaPayload = buildCreatorOptionalMedia({
        useBackgroundMusic: submitUseBackgroundMusic,
        useCover: submitUseCover,
        musicUrl: selected.music.audioUrl,
        musicVolume,
        coverTemplateId: selected.coverTemplate.id,
        coverUrl,
      });
      const {
        backgroundMusic,
      ...
      const shanjianData = {
        ...
        processRules: { watermarkShow: true, firstFrameCover },
        bgmusic,
        useBackgroundMusic: optionalMediaPayload.useBackgroundMusic,
        useCover: optionalMediaPayload.useCover,
      };
      if (isCustomMixcut) {
      ...
      const activeDraftPayload = activeDraftId ? {
        ...
        temporaryId: activeDraftId,
        temporary_id: activeDraftId,
        draftRecordId: activeDraftId,
        draft_record_id: activeDraftId,
        sourceDraftId: activeDraftId,
        source_draft_id: activeDraftId,
        fromDraft: !isDraft,
        submitFromDraft: !isDraft,
      } : {};
      const payload = {
        title: form.title.trim(), topic: form.topic.trim(), tags: form.topic.trim(), script: scriptContent, content: scriptContent,
        cover: optionalMediaPayload.coverUrl, coverUrl: optionalMediaPayload.coverUrl, voiceId: selected.voice.id, voiceName: selected.voice.title, speakerId: selected.voice.id,
        speakerExtra, speaker_extra: speakerExtra, coverTemplateId, coverTemplateName: submitUseCover ? selected.coverTemplate.title : '',
        ...
        is_draft: Boolean(isDraft), bgmusic, useBackgroundMusic: optionalMediaPayload.useBackgroundMusic, useCover: optionalMediaPayload.useCover, shanjianData,
  ...
  const selectorRows = [
    ...
    ...(useCover ? [{ key: 'coverTemplate', label: '视频封面模板', hint: '请选择视频封面模板', icon: Image }] : []),
    ...(useBackgroundMusic ? [{ key: 'music', label: '背景音乐', hint: '不选择时自动匹配', icon: Music2 }] : []),
  ];
  ...
  const optionalMediaControls = (
    <div className="video-creator-option-toggles">
      <article>
        <span><Music2 size={19} /></span>
        <div><strong>背景音乐</strong><small>{useBackgroundMusic ? (selected.music.title || '自动匹配背景音乐') : '关闭'}</small></div>
        <button className={useBackgroundMusic ? 'is-on' : ''} type="button" role="switch" aria-checked={useBackgroundMusic} aria-label="是否开启背景音乐" onClick={() => toggleOptionalMedia('useBackgroundMusic')}>
          <i /><em>{useBackgroundMusic ? '开启' : '关闭'}</em>
        </button>
      </article>
      <article>
        <span><Image size={19} /></span>
        <div><strong>封面</strong><small>{useCover ? (selected.coverTemplate.title || '封面图片') : '关闭'}</small></div>
        <button className={useCover ? 'is-on' : ''} type="button" role="switch" aria-checked={useCover} aria-label="是否使用封面" onClick={() => toggleOptionalMedia('useCover')}>
          <i /><em>{useCover ? '开启' : '关闭'}</em>
        </button>
      </article>
    </div>
  );
      ...
      {!token ? <div className="video-empty-state"><UserRound size={38} /><strong>登录后开始制作</strong><p>登录后才能读取数字人、声音、素材并提交制作任务。</p><button className="primary-button" onClick={onLogin}>登录</button></div> : <>
        <div className="video-creator-layout">
          <main className="video-creator-main">
            ...
            <section className="video-creator-section"><div className="video-creator-section__head"><span>02</span><div><h2>选择配置</h2><p>{isCustomMixcut ? '选择声音和包装混剪模板，并设置背景音乐和封面。' : isMixed ? '选择声音、混剪剪辑模板，并设置背景音乐和封面。' : '选择数字人、声音、视频包装，并设置背景音乐和封面。'}</p></div>{shouldShowPackagingPresetPicker(needsHuman) && <button type="button" className="video-creator-preset-button" onClick={() => setDialogType('preset')}>{loadingResources ? <RefreshCw className="is-spinning" size={16} /> : <Layers3 size={16} />}选择已配置</button>}</div>{optionalMediaControls}<div className="video-creator-select-list">{selectorRows.map(({ key, label, hint, icon: Icon }) => { const value = selected[key]; const preview = value.cover; return <button key={key} onClick={() => setDialogType(key)}><i><Icon size={19} /></i><span><strong>{label}</strong><small>{value.title || hint}</small></span>{preview && <img src={preview} alt="" />}{key === 'music' && value.audioUrl && <audio src={value.audioUrl} controls onClick={(event) => event.stopPropagation()} />}<ChevronRight size={18} /></button>; })}</div>{isCustomMixcut && useCover && <div className="video-creator-upload-grid video-creator-cover-grid">{coverPanel}</div>}</section>
            <section className="video-creator-section"><div className="video-creator-section__head"><span>03</span><div><h2>{isCustomMixcut ? '文案分镜' : useCover ? '封面与素材' : '视频素材'}</h2><p>{isCustomMixcut ? '分镜需要字幕文本，素材可挂在任意分镜，但至少选择 1 个。' : '图片按 2 秒计，单个视频小于 60 秒，总时长不超过 5 分钟。'}</p></div></div>{!isCustomMixcut && <div className={`video-creator-upload-grid${useCover ? '' : ' is-single'}`}>{useCover && coverPanel}<div className="video-creator-material-actions"><h3>视频素材 <em>必填</em></h3><div><button onClick={() => setDialogType('material')}><Library size={18} />选择素材</button><label><Upload size={18} />本地上传<input type="file" multiple accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime" onChange={chooseLocalMaterials} /></label></div><small>已选 {materials.length} 个 · 总时长 {formatDuration(materialDuration)}</small></div></div>}{isCustomMixcut ? <div className="video-scene-builder"><div className="video-scene-builder__toolbar"><button onClick={splitScriptToScenes}><Sparkles size={16} />按文案拆分</button><button onClick={addScene}><Plus size={16} />新增分镜</button><span>{scenes.length} 个分镜 · {sceneMaterialCount} 个素材 · {formatDuration(materialDuration)}</span></div><div className="video-scene-list">{scenes.map((scene, index) => <article className="video-scene-card" key={scene.id}><header><span>分镜 {index + 1}</span><button onClick={() => removeScene(scene.id)} disabled={scenes.length <= 1} aria-label="删除分镜"><Trash2 size={15} /></button></header><label><span>字幕文本 <em>必填</em></span><textarea value={scene.content} onChange={(event) => changeSceneContent(scene.id, event.target.value)} onKeyDown={(event) => handleSceneKeyDown(event, scene.id)} placeholder="按回车从光标处分成两个分镜" /></label><div className="video-scene-actions"><button onClick={() => { setActiveSceneId(scene.id); setDialogType('material'); }}><Library size={16} />选择素材</button><label><Upload size={16} />本地上传<input type="file" multiple accept="image/jpeg,image/png,image/webp,video/mp4,video/quicktime" onChange={(event) => chooseLocalMaterials(event, scene.id)} /></label><small>{(scene.materials || []).length} 个素材</small></div>{(scene.materials || []).length > 0 && <div className="video-creator-material-grid video-scene-material-grid">{scene.materials.map((material) => <article key={`${scene.id}-${material.id}`}><span>{material.type === 'video' && material.file ? <video src={material.previewUrl} muted playsInline /> : material.previewUrl ? <img src={material.previewUrl} alt="" /> : <Video size={24} />}</span><div><strong>{material.title}</strong><small>{material.type === 'video' ? `视频 · ${formatDuration(material.duration)}` : '图片 · 2 秒'}</small></div><button onClick={() => removeSceneMaterial(scene.id, material.id)} aria-label="删除素材"><X size={16} /></button></article>)}</div>}</article>)}</div></div> : materials.length > 0 && <div className="video-creator-material-grid">{materials.map((material) => <article key={material.id}><span>{material.type === 'video' && material.file ? <video src={material.previewUrl} muted playsInline /> : material.previewUrl ? <img src={material.previewUrl} alt="" /> : <Video size={24} />}</span><div><strong>{material.title}</strong><small>{material.type === 'video' ? `视频 · ${formatDuration(material.duration)}` : '图片 · 2 秒'}</small></div><button onClick={() => removeMaterial(material.id)} aria-label="删除素材"><X size={16} /></button></article>)}</div>}</section>
          </main>
          <aside className="video-creator-summary"><span>PRODUCTION SUMMARY</span><h2>制作确认</h2><dl><div><dt>标题</dt><dd>{form.title || '未填写'}</dd></div>{needsHuman && <div><dt>数字人</dt><dd>{selected.human.title || '未选择'}</dd></div>}<div><dt>声音</dt><dd>{selected.voice.title || '未选择'}</dd></div><div><dt>{isMixed ? '混剪模板' : isCustomMixcut ? '包装模板' : '视频包装'}</dt><dd>{selected.videoTemplate.title || '未选择'}</dd></div><div><dt>封面</dt><dd>{useCover ? (selected.coverTemplate.title || '未选择') : '关闭'}</dd></div><div><dt>背景音乐</dt><dd>{useBackgroundMusic ? (selected.music.title || '系统自动匹配') : '关闭'}</dd></div>{isCustomMixcut && <div><dt>分镜</dt><dd>{scenes.length} 个</dd></div>}<div><dt>素材</dt><dd>{isCustomMixcut ? sceneMaterialCount : materials.length} 个 / {formatDuration(materialDuration)}</dd></div></dl>{uploadProgress && <div className="video-creator-uploading"><RefreshCw className="is-spinning" size={17} />{uploadProgress}</div>}{message && <div className={`video-list-message ${/失败|请|不能|未返回|最多/.test(message) ? 'is-error' : ''}`}>{message}</div>}<div className="video-creator-submit"><button className="outline-button" onClick={() => submit(true)} disabled={Boolean(busy)}>{busy === 'draft' ? '暂存中…' : '暂存'}</button><button className="primary-button" onClick={() => submit(false)} disabled={Boolean(busy)}><Sparkles size={17} />{busy === 'submit' ? '提交中…' : '提交制作'}</button></div><p>提交后会进入制作队列，可在 Video Studio 查看进度。</p></aside>
        </div>
      </>}
      {dialogType && (() => {
        const materialResourceKey = getCreatorMaterialSource(materialSource).resourceKey;
        const resourceKey = dialogType === 'material' ? materialResourceKey : dialogType;
        return <VideoCreatorDialog type={dialogType} titleOverride={isMixed && dialogType === 'videoTemplate' ? '选择混剪剪辑模板' : isCustomMixcut && dialogType === 'videoTemplate' ? '选择包装混剪模板' : ''} options={resources[resourceKey] || []} selected={dialogType === 'material' ? activeSceneMaterials : selected[dialogType]} loading={loadingResources} hasMore={resourcePaging[resourceKey]?.hasMore || false} loadingMore={resourcePaging[resourceKey]?.loadingMore || false} loadMessage={resourcePaging[resourceKey]?.message || ''} materialSource={materialSource} onMaterialSourceChange={setMaterialSource} onClose={() => { setDialogType(''); setActiveSceneId(''); }} onSelect={(option) => chooseResource(dialogType, option)} onLoadMore={() => loadMoreCreatorResources(resourceKey)} />;
      })()}
    </div>
  );
}
...
const buildTypedPrompt = (prompt, type, agent = {}) => {
  ...
  if (normalizeInstructionType(type).value === '音乐创作') {
    return [
      ...
      '字段标签固定保留为上述中文标签，字段内容使用与“用户需求”相同的主要语言；用户使用中文时，全部内容使用中文。',
          ...
          {isPublicInfo ? (
            ...
            active === 'trends' ? (
              ...
              <>
                ...
                {aiVideoPackagingSource && (
                  <AIVideoPackagingPage
                    ...
                    language={language}
                    onBack={() => {
                      const returnTo = aiVideoPackagingSource?.returnTo;
                      setAIVideoPackagingSource(null);
                      if (returnTo === 'video') {
                        setActive('video');
                        syncWorkspacePageQuery('video');
              ...
              <VideoStudioPage
                ...
                onCreatePackaging={() => {
                  setVideoStudioInitialSection('realman');
                  setAIVideoPackagingSource({ origin: 'local', returnTo: 'video' });
                  syncWorkspacePageQuery('ai-video');
                  setActive('ai-video');
                  window.scrollTo({ top: 0, behavior: 'smooth' });
                }}
