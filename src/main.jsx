...
const copy = {
  en: {
    workspace: 'AI Content Production Workspace',
    subline: 'Turn trends, scripts, avatars, voices, music, images, and templates into publish-ready videos.',
  ...
  zh: {
    workspace: 'AI 内容生产工作台',
    subline: '把热点、脚本、数字人、声音、音乐、图片和模板串成可发布的视频。',
    trial: '免费体验权益',
    cta: '新建视频',
    search: '搜索热点、脚本、资产',
    workflow: '推荐流程',
  ...
  ja: {
    workspace: 'AI コンテンツ制作ワークスペース',
    subline: 'トレンド、台本、アバター、音声、音楽、画像、テンプレートを動画制作へつなぎます。',
    trial: '無料トライアル特典',
    cta: '動画を作成',
  ...
  ko: {
    workspace: 'AI 콘텐츠 제작 워크스페이스',
    subline: '트렌드, 스크립트, 디지털 휴먼, 음성, 음악, 이미지, 템플릿을 영상 제작으로 연결합니다.',
    trial: '무료 체험 혜택',
    cta: '새 영상',
  ...
  de: {
    workspace: 'KI Content Production Workspace',
    subline: 'Verbinde Trends, Skripte, Avatare, Stimmen, Musik, Bilder und Vorlagen zu fertigen Videos.',
  ...
  ru: {
    workspace: 'Рабочее пространство AI-контента',
    subline: 'Собирайте тренды, сценарии, аватары, голоса, музыку, изображения и шаблоны в готовые видео.',
...
const trialBenefits = [
  { title: '1 Digital Human', image: '/yixiu-assets/trial-human.jpg' },
  { title: '1 Voice', image: '/yixiu-assets/trial-voice.jpg' },
  { title: '3 AI Videos', image: '/yixiu-assets/trial-video.jpg' },
];
...
const actionCards = [
  {
    id: 'trends',
    title: 'Hot Tracking',
    text: 'Aggregate global and media trends for script ideas.',
    icon: TrendingUp,
    color: '#00a884',
  ...
  },
  {
    ...
    text: 'Instruction sets for copy, music, image, and video workflows.',
    icon: BrainCircuit,
    color: '#1b7f68',
  ...
  },
  {
    id: 'assets',
    title: 'Digital Human',
    text: 'Train and manage presenter avatars for video production.',
    icon: UserRound,
    color: '#1f9d76',
  ...
  },
  {
    id: 'assets',
    title: 'Voice Clone',
    text: 'Create multilingual voice assets with consent-first flows.',
    icon: Mic2,
    color: '#d58b2a',
    image: '/yixiu-assets/voice-clone.png',
  },
  {
    id: 'music',
    title: 'AI Music',
    text: 'Generate prompt songs, lyrics songs, instrumentals, and music videos.',
    icon: Music2,
    color: '#0e8fbd',
    image: '/yixiu-assets/media-hot.png',
  },
  {
    id: 'image',
    title: 'Image Generation',
    text: 'Create campaign images and convert selected images into avatars.',
    icon: Image,
    color: '#64748b',
  ...
  },
  {
    id: 'video',
    title: 'Video Studio',
    text: 'Combine scripts, avatars, voices, templates, covers, materials, and music.',
    icon: FileVideo,
    color: '#007d68',
  ...
  },
  {
    id: 'materials',
    title: 'Materials',
    text: 'Upload, preview, select, and reuse creative assets.',
    icon: Library,
    color: '#5b6d5c',
...
const usageItems = [
  { name: 'Digital Human', remaining: 1, total: 3, icon: UserRound },
  { name: 'Voice', remaining: 2, total: 5, icon: Mic2 },
  { name: 'Video', remaining: 7, total: 20, icon: Video },
  { name: 'AI Music', remaining: 12, total: 30, icon: Music2 },
];
...
const workflowSteps = [
  { no: '01', title: 'Capture trend', text: 'Find a topic and save it into the script flow.' },
  { no: '02', title: 'Generate script', text: 'Use an instruction set to draft copy, lyrics, or video structure.' },
  { no: '03', title: 'Select assets', text: 'Choose digital human, voice, music, templates, cover, and materials.' },
  { no: '04', title: 'Produce and publish', text: 'Create a task, review output, publish now or schedule.' },
];
...
function ActionGrid({ active, onSelect }) {
  return (
    <section className="action-grid" aria-label="AI asset workbench">
...
function UsagePanel({ language }) {
  ...
  return (
    <aside className="usage-panel">
      ...
      <button className="outline-button">Upgrade plan</button>
    </aside>
  );
}
  ...
  return (
    <section className="preview-panel">
      <div className="panel-head">
        <div>
          <h2>{t.preview}</h2>
          <p>Web replacement for the mini program video creation flow</p>
        </div>
      ...
      <div className="video-builder">
        ...
        <div className="builder-list">
          {['Script', 'Digital Human', 'Voice', 'Template', 'Cover', 'Music'].map((item, index) => (
            <div className="builder-row" key={item}>
              <span>{item}</span>
              <strong>{index < 3 ? 'Selected' : 'Choose'}</strong>
            </div>
          ))}
        </div>
      </div>
...
function AssetStudioPage({ authVersion, language, onLogin, onOpenInfo, onUseAsset, initialImageId = '', initialMode = '' }) {
  ...
  const submitVideoTraining = async () => {
    ...
    const { authorizationVideoId, authVideoUrl } = await resolveAuthVideo();
    const result = await apiFetch('/api/aihuman/train', {
      ...
      body: omitEmpty({
        ...
        authorization_text: authorizationLines.join('\n'),
        authorizationText: authorizationLines.join('\n'),
      }),
    });
...
function VideoCreatorPage({ authVersion, usePrefill, onBack, onLogin, onCreated }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [ownHumans, commonHumans, ownVoices, commonVoices, videoTemplates, coverTemplates, materialResult, musicResult, userResult] = await Promise.all([
        ...
        apiFetch('/api/user/info', { timeoutMs: 10000 }),
      ]);
      if (ignore) return;
  ...
  const validate = (isDraft) => {
    ...
    if (!selected.human.id) return '请选择数字人形象';
    if (!selected.voice.id) return '请选择声音';
    if (!selected.videoTemplate.id) return '请选择视频包装模板';
  ...
  const submit = async (isDraft = false) => {
    ...
    try {
      ...
      for (let index = 0; index < materials.length; index += 1) {
        ...
        submitMaterials.push({ type: material.type, fileUrl: submitUrl, soundSwitch: false });
      }
      ...
      const speakerExtra = { speedRatio: Math.min(2, Math.max(0.5, Number(selected.voice.speed) || 1)) };
      const backgroundMusic = { audioSwitch: true, audioUrl: selected.music.audioUrl || '', url: selected.music.audioUrl || '' };
      const shanjianData = {
      ...
      const payload = {
        ...
        videoTemplateId: selected.videoTemplate.id, videoTemplateName: selected.videoTemplate.title, scene: 'digital_human_video', templateScene: 'virtualman',
        materials: submitMaterials, is_draft: Boolean(isDraft), bgmusic: { url: selected.music.audioUrl || '' }, shanjianData,
        aiHumanId: selected.human.id, aiHumanName: selected.human.title, virtualmanId: selected.human.id, virtualmanName: selected.human.title,
        ...(draftId ? { id: draftId, draft_id: draftId } : {}),
      };
      setUploadProgress(isDraft ? '正在暂存…' : '正在提交制作任务…');
      const result = await apiFetch('/api/video/production/create', { method: 'POST', body: payload, timeoutMs: 180000 });
  ...
  const selectorRows = [
    { key: 'human', label: '数字人形象', hint: '请选择已完成的形象', icon: UserRound },
    { key: 'voice', label: '声音', hint: '请选择已完成的声音', icon: Mic2 },
    { key: 'videoTemplate', label: '视频包装模板', hint: '请选择视频包装模板', icon: Video },
      ...
      {!token ? <div className="video-empty-state"><UserRound size={38} /><strong>登录后开始制作</strong><p>登录后才能读取数字人、声音、素材并提交制作任务。</p><button className="primary-button" onClick={onLogin}>登录</button></div> : <>
        ...
        <div className="video-creator-layout">
          <main className="video-creator-main">
            <section className="video-creator-section"><div className="video-creator-section__head"><span>01</span><div><h2>基础内容</h2><p>标题和话题用于成片包装，文案会由数字人朗读。</p></div></div><div className="video-creator-fields"><label><span>标题 <em>必填</em></span><input maxLength={80} value={form.title} onChange={(event) => updateForm('title', event.target.value)} placeholder="请输入视频标题" /><small>{form.title.length}/80</small></label><label><span>话题 <em>必填</em></span><input value={form.topic} onChange={(event) => updateForm('topic', event.target.value)} placeholder="例如：同城获客、门店活动" /></label><label className="is-wide"><span>文案 <em>必填</em></span><textarea maxLength={2000} value={form.script} onChange={(event) => updateForm('script', event.target.value)} placeholder="请输入数字人口播文案" /><small>{form.script.length}/2000</small></label></div></section>
            <section className="video-creator-section"><div className="video-creator-section__head"><span>02</span><div><h2>选择配置</h2><p>使用已有资源，也可以一键应用团队预设。</p></div>{resources.preset.length > 0 && <button className="video-creator-preset-button" onClick={() => setDialogType('preset')}><Layers3 size={16} />选择已配置</button>}</div><div className="video-creator-select-list">{selectorRows.map(({ key, label, hint, icon: Icon }) => { const value = selected[key]; const preview = value.cover; return <button key={key} onClick={() => setDialogType(key)}><i><Icon size={19} /></i><span><strong>{label}</strong><small>{value.title || hint}</small></span>{preview && <img src={preview} alt="" />}{key === 'music' && value.audioUrl && <audio src={value.audioUrl} controls onClick={(event) => event.stopPropagation()} />}<ChevronRight size={18} /></button>; })}</div></section>
  ...
  return (
    <section className="workflow-section">
      <div className="section-title">
        <h2>{t.workflow}</h2>
        <p>The original mini program flow mapped to a web console.</p>
      </div>
            ...
            <>
              ...
              <div className="content-grid">
                <div className="content-main">
                  ...
                  <PreviewPanel language={language} activeMode={activeMode} setActiveMode={setActiveMode} />
