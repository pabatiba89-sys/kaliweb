...
发布话题在用户点击提交时生成唯一快照：按页面输入顺序解析，移除话题内部空白，并按第一次出现的位置去重。页面话题预览、Kali 发布请求和本地 `/postVideo` 的 `tags` 必须复用同一数组，不得重新读取任务记录中的旧 `tags`。当前只读后台的 `utils/tags.py` 仍会为“老陆”系列账号在第三位插入“知识播种计划”；若要求 Notion 中的 Tags 也与页面完全一致，后台需移除或显式关闭这条账号专属规则。
