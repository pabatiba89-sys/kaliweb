...
1. `POST /uploadFromUrl` 下载成片到本机。
2. `GET /getAccounts?nocheck=1` 读取本地账号，并按 Kali 发布账号名称精确匹配启用账号。
3. 按平台类型分组调用 `POST /postVideo`，同名账号对应多个平台时分别发布。
...
发布按钮在调用 Kali 接口前必须先请求 `/getAccounts?nocheck=1` 检测本地服务。检测不到 5409 时中止本次提交，保留用户已填写的发布配置，并展示本地工具启动说明、安装下载入口和“检测并继续发布”操作；重新检测成功后才创建 Kali 发布任务并继续上述本地发布链路。
